...
local function RestackBagCheck( loc_id, bag_id )
	...
	if bag_id == REAGENTBANK_CONTAINER and not IsReagentBankUnlocked( ) then
...
local function FindPartialStack( loc_id, cl, cb, bp, cs, id )
	...
	--ArkInventory.Output( "FindPartialStack( ", loc_id, " / ", cl, ".", cb, ".", cs, " / ", id, " )" )
	...
	if cl == ArkInventory.Const.Location.Bag or cl == ArkInventory.Const.Location.Bank then
		...
		if cb == REAGENTBANK_CONTAINER then
			...
			return FindStack( ArkInventory.Const.Location.Bank, cl, cb, bp, -1, id )
								...
								if info.equiploc ~= "INVTYPE_BAG" then
									...
									if check_item then
										...
										if bit.band( it, ct ) > 0 then
											--ArkInventory.Output( "prof> ", ArkInventory.Global.Location[loc_id].Name, ".", bag_id, ".", slot_id, " " , h )
											return abort, recheck, true, bag_id, slot_id
										end
					...
					if select( 3, GetContainerItemInfo( bag_id, slot_id ) ) then
						...
						if h then
							...
							if num < info.stacksize then
								...
								--ArkInventory.Output( "partial stack of ", num, "x", h, " found at ", bag_id, ".", slot_id )
